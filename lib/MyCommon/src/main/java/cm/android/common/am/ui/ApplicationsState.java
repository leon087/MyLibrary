        ...
        boolean ensureIconLocked(Context context, PackageManager pm) {
            if (this.icon == null) {
                if (this.apkFile.exists()) {
                    ...
                    this.mounted = false;
                    this.icon = context.getResources()
...
//        }
    ...
    void doResumeIfNeededLocked() {
        ...
        for (int i = 0; i < mApplications.size(); i++) {
            ...
            if (!info.enabled) {
                int enabledSetting = mContext.getPackageManager().getApplicationEnabledSetting(
                        info.packageName);
                //ggg
                ...
                case MSG_LOAD_SIZES: {
                    synchronized (mEntriesMap) {
                        ...
                        if (mCurComputingSizePkg != null) {
                            if (DEBUG_LOCKING) {
                                Log
                                        .v(TAG, "MSG_LOAD_SIZES releasing: currently computing");
                            }
                            return;
                        }
...
//                                    mPm.getPackageSizeInfo(mCurComputingSizePkg, mStatsObserver);
                                ...
                                if (DEBUG_LOCKING) {
                                    Log
                                            .v(TAG, "MSG_LOAD_SIZES releasing: now computing");
                                }
                                return;
                            }
                        }
