...
import cm.android.applications.InterestingConfigChanges;

/**
 * Keeps track of information about all installed applications, lazy-loading as needed.
 */
public class ApplicationsState {
    /**
     * Value for {@link #flags}: true if the application is currently installed for the calling
     * user.
     */
    ...
    public static final Comparator<AppEntry> ALPHA_COMPARATOR = new Comparator<AppEntry>() {
        ...
        public int compare(AppEntry object1, AppEntry object2) {
            final boolean normal1 = object1.info.applicationInfo.enabled
                    && (object1.info.applicationInfo.flags & FLAG_INSTALLED) != 0;
            final boolean normal2 = object2.info.applicationInfo.enabled
                    && (object2.info.applicationInfo.flags & FLAG_INSTALLED) != 0;
            if (normal1 != normal2) {
                return normal1 ? -1 : 1;
            }
    ...
    private ApplicationsState(Application app) {
        ...
        /**
         * This is a trick to prevent the foreground thread from being delayed. The problem is that
         * Dalvik monitors are initially spin locks, to keep them lightweight. This leads to unfair
         * contention -- Even though the background thread only holds the lock for a short amount of
         * time, if it keeps running and locking again it can prevent the main thread from acquiring
         * its lock for a long time... sometimes even > 5 seconds (leading to an ANR). Dalvik will
         * promote a monitor to a "real" lock if it detects enough contention on it. It doesn't
         * figure this out fast enough for us here, though, so this little trick will force it to
         * turn into a real lock immediately.
         */
    ...
    void doResumeIfNeededLocked() {
        ...
        // }
        mPackages = mPm.getInstalledPackages(mRetrieveFlags);
        if (mPackages == null) {
    ...
    AppEntry getEntry(String packageName) {
        ...
        synchronized (mEntriesMap) {
            ...
            if (entry == null) {
                for (int i = 0; i < mPackages.size(); i++) {
                    PackageInfo info = mPackages.get(i);
    ...
    int indexOfApplicationInfoLocked(String pkgName) {
        for (int i = mPackages.size() - 1; i >= 0; i--) {
            if (mPackages.get(i).packageName.equals(pkgName)) {
                return i;
            }
        }
        return -1;
    ...
    void addPackage(String pkgName) {
        try {
            synchronized (mEntriesMap) {
                ...
                // mRetrieveFlags);
                PackageInfo packageInfo = mPm.getPackageInfo(pkgName,
                        mRetrieveFlags);
    ...
    AppEntry getEntryLocked(PackageInfo info) {
    ...
    public static class AppEntry extends SizeInfo {
        ...
        AppEntry(Context context, PackageInfo packageInfo, long id) {
        ...
        void ensureLabel(Context context) {
            if (this.label == null || !this.mounted) {
                if (!this.apkFile.exists()) {
                    ...
                    this.mounted = true;
                    CharSequence label = info.applicationInfo.loadLabel(context
                            .getPackageManager());
        ...
        boolean ensureIconLocked(Context context, PackageManager pm) {
            if (this.icon == null) {
                if (this.apkFile.exists()) {
                    this.icon = this.info.applicationInfo.loadIcon(pm);
                    return true;
                } else {
                    this.mounted = false;
    ...
    public class Session {
        ...
        void handleRebuildList() {
            ...
            synchronized (mEntriesMap) {
                apps = new ArrayList<PackageInfo>(mPackages);
            }
            ...
            for (int i = 0; i < apps.size(); i++) {
                ...
                if (filter == null || filter.filterApp(info)) {
                    synchronized (mEntriesMap) {
                        ...
                        AppEntry entry = getEntryLocked(packageInfo);
            ...
            switch (msg.what) {
                ...
                case MSG_LOAD_ENTRIES: {
                    ...
                    synchronized (mEntriesMap) {
                        ...
                        for (int i = 0; i < mPackages.size() && numDone < 6; i++) {
                            if (!mRunning) {
                                mRunning = true;
